...
@section('content')
	...
	<div class="row">
        <div class="col-lg-12">
            <h1 class="filter page-header">Data Surat Keterangan Kelakuan Baik</h1>
        </div>
    </div>
		...
		@foreach($skkb as $index => $row)
			...
			?>
				<tr>
					...
					<td class="text-center"><a class="btn btn-primary" style="width: 70px" href="/skkb/{{$row->id}}">Detail</a>
					<a id="hapus_kematian" class="btn btn-danger" style="width: 70px" href="/skkb/{{$row->id}}/delete">Hapus</a>
					</td>
				</tr>
		@endforeach
		</tbody>
	...
	<center>
		<form action="/skkb/download" method="post" autocomplete="off">
			{{ csrf_field() }}
			<input type="hidden" name="surat_download" value="{{ $skkb_download }}" required>
