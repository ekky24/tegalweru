...
@section('content')
	...
	<div class="row">
        <div class="col-lg-12">
            <h1 class="filter page-header">Data Surat Keterangan Wali Nikah</h1>
        </div>
    </div>
			...
			?>
				<tr>
					...
					@if(strlen($row->keperluan) > 40)
						<td>{{ substr($row->get_penduduk->nama, 0, 40) . "..."}}</td>
					@else
						<td>{{ $row->get_penduduk->nama }}</td>
					@endif

					<td>{{ $row->nama_nikah }}</td>
					<td>{{ $row->get_penerbit->nama }}</td>
					<td>{{ strtoupper($bulan_arr[$waktu->month - 1] . " " . $waktu->format('Y')) }}</td>
					<td class="text-center"><a class="btn btn-primary" style="width: 70px" href="/skwn/{{$row->id}}">Detail</a>
					<a id="hapus_kematian" class="btn btn-danger" style="width: 70px" href="/skwn/{{$row->id}}/delete">Hapus</a>
					</td>
				</tr>
		@endforeach
		</tbody>
	...
	<center>
		<form action="/skwn/download" method="post" autocomplete="off">
			{{ csrf_field() }}
			<input type="hidden" name="surat_download" value="{{ $skwn_download }}" required>
