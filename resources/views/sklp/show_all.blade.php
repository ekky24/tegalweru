...
@section('content')
	...
	<div class="row">
        <div class="col-lg-12">
            <h1 class="filter page-header">Data Surat Keterangan Lunas PBB</h1>
        </div>
    </div>
		...
		@foreach($sklp as $index => $row)
			...
			?>
				<tr>
					...
					<td class="text-center"><a class="btn btn-primary" style="width: 70px" href="/sklp/{{$row->id}}">Detail</a>
					<a id="hapus_kematian" class="btn btn-danger" style="width: 70px" href="/sklp/{{$row->id}}/delete">Hapus</a>
					</td>
				</tr>
		@endforeach
		</tbody>
	...
	<center>
		<form action="/sklp/download" method="post" autocomplete="off">
			{{ csrf_field() }}
			<input type="hidden" name="surat_download" value="{{ $sklp_download }}" required>
